...
def run_gpt_prompt_task_decomp(persona, task, duration, test_input=None, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        # The prompt continues a numbered list whose items look like
        #   2) Klaus is reviewing notes. (duration in minutes: 15, minutes left: 45)
        # The first item may come back without its "1) Klaus is" prefix, since
        # that part is already in the prompt.
        firstname = persona.scratch.get_str_firstname()
        cr = []
        for line in gpt_response.splitlines():
            if "(duration in minutes:" not in line:
                continue
            task, rest = line.split("(duration in minutes:", 1)
            task = re.sub(r"^\s*\d+[\).]\s*", "", task).strip()
            other = re.match(r"^([A-Z][\w'-]*) is ", task)
            if other and other.group(1) != firstname:
                # The model restated the few-shot example (e.g. "Kelly is ...").
                continue
            if task.lower().startswith(f"{firstname.lower()} is "):
                task = task[len(firstname) + 4:]
            elif task.lower().startswith(f"{firstname.lower()} "):
                # e.g. "Isabella gets dressed"
                task = task[len(firstname) + 1:]
            task = task.strip().rstrip(".").strip()
            duration = re.match(r"\s*(\d+)", rest)
            if task and duration and int(duration.group(1)) > 0:
                cr += [[task, int(duration.group(1))]]
        return cr
  ...
  """
    ...
    if not fin_output:
        # The first subtask alone is longer than the whole task.
        fin_output = [[output[0][0], 0]]
    fin_output[-1][1] += duration - ftime_sum
    output = fin_output
...
def run_gpt_prompt_action_sector(
    ...
    def __func_clean_up(gpt_response, prompt=""):
      ...
      # Stop at the first newline or period to prevent sentence regurgitation
      if "\n" in cleaned:
      ...
      cleaned = cleaned.replace("Based on the rules", "") # Strip the specific hallucination
    ...
    def __func_validate(gpt_response, prompt=""): 
      try:
          ...
          if len(cleaned.strip()) < 1: 
              return False
          if len(cleaned) > 50:  # A room/sector name should never be longer than 50 chars
              return False
          return True
      except:
          return False
...
def run_gpt_prompt_pronunciatio(action_description, persona, verbose=False):
    ...
    # Fallback: if all paths failed, return fail_safe instead of None
    return fail_safe, [fail_safe, prompt, gpt_param, prompt_input, fail_safe]


def clean_up_event_triple(gpt_response, subject=""):
    """
    The event triple prompts end with "Output: (<subject>," and expect only the
    remaining "<predicate>, <object>)". Chat models tend to answer with the whole
    triple, or re-answer every few-shot example before getting to ours, so we
    find the line about our subject and normalize it down to [predicate, object].
    """
    cr = gpt_response.strip()
    lines = [i for i in cr.splitlines() if "," in i]
    subject_lines = [i for i in lines
                     if subject and f"({subject.lower()}," in i.lower()]
    if subject_lines:
        # When the model restates the examples, ours comes last.
        cr = subject_lines[-1]
    elif lines:
        cr = lines[0]
    cr = cr.split("(")[-1].split(")")[0]
    parts = [i.strip().strip("\"'").strip() for i in cr.split(",")]
    parts = [i for i in parts if i]
    if len(parts) == 3 or (subject and parts and parts[0].lower() == subject.lower()):
        # The model repeated the subject; drop it.
        parts = parts[1:]
    if len(parts) == 1 and " " in parts[0]:
        # Missing comma, e.g. "brew coffee)".
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        return clean_up_event_triple(gpt_response, persona.name)
    ...
    def get_fail_safe(persona):
        # Only the (predicate, object) pair; the subject is prepended below.
        fs = ("is", "idle")
        return fs
...
def run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False):
    ...
    # If all paths failed, return the fail_safe instead of None
    return fail_safe, [fail_safe, prompt, gpt_param, prompt_input, fail_safe]
...
def run_gpt_prompt_act_obj_event_triple(
    ...
    def __func_clean_up(gpt_response, prompt=""):
        return clean_up_event_triple(gpt_response, act_game_object)
